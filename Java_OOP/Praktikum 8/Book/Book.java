...
//      throws InvalidBookException
  public Book(String title, String author, String isbn, int publicationYear, int copies) throws InvalidBookException{
    ...
    if(author == null || author.trim().isEmpty()){
      throw new InvalidBookException("Invalid author", title, "author");
    }

    String[] authorWords = author.trim().split("\\s+");
    if(authorWords.length < 2){
      throw new InvalidBookException("Author must be 2 words", title, "author", author);
    }

    int currentYear = java.time.Year.now().getValue();
    if (publicationYear < 1000 || publicationYear > currentYear) {
      throw new InvalidBookException("Publication year must be between 1000 and " + currentYear, 
      title, "publicationYear", String.valueOf(publicationYear));
    }

    if(copies < 0){
      throw new InvalidBookException("Copies negative", title, "copies");
    }

    if(!isValidISBN(isbn)){
      throw new InvalidBookException("Invalid ISBN", title, "isbn");
    }
...
//      * Otherwise: return false
  public boolean isValidISBN(String isbn){
    ...
    if(cleanISBN.length() == 10){return isValidISBN10(cleanISBN);}
    else if(cleanISBN.length() == 13){return isValidISBN13(cleanISBN);}
    else{return false;}
  }
...
//      * Kurangi availableCopies
    public void borrowBook() throws BookNotAvailableException{
      if(availableCopies <= 0){
        throw new BookNotAvailableException("No book bruh", title, 1, availableCopies);
      }
...
public void returnBook() throws InvalidBookException {
  if(availableCopies >= copies){
    throw new InvalidBookException("Too much book",title, "availableCopies", String.valueOf(availableCopies));
  }
