...
public class VideoPost extends Post implements  Engageable, Shareable {
    ...
    private int totalScore() {
      int total = (int) (baseScore() + getViews()*0.5 + getDurationSec()/30);
      total = (useAI) ? total - 10: total;
    ...
    public int compareEngagement(Engageable other) {
      VideoPost otherP = (VideoPost) other;
    ...
    public void shareTo(String target) {
        System.out.println(String.format("Video by @%s shared to %s", getAuthor(), target));
        share();
    }
  }
