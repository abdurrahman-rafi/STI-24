...
public class UserSubscriber implements Subscriber{
    ...
    public void addCategoryPreference(String agencyName, String category, DeliveryMethod method) {
        ...
        // if(s == null){
        //     System.out.println(String.format("Pengguna %s tidak dapat menambahkan kategori '%s' ke agensi %s.", getName(), category, agencyName));
        // }else{
        //     s.setDeliveryMethod(method);
        //     s.addCategory(category);
        //     preferences.add(s);
        //     System.out.println(String.format("Pengguna %s menambahkan kategori '%s' untuk agensi %s dengan metode %s.", getName(), category, agencyName, s.getDeliveryMethod()));
        // }
        s.setDeliveryMethod(method);
    ...
    public void removeCategoryPreference(String agencyName, String category) {
        ...
        AbstractAgency a = AbstractAgency.findByName(agencyName);
        // int index = a.subscribers.indexOf();
        if(s == null){
            System.out.println(String.format("Pengguna %s belum memiliki preferensi untuk agensi %s, kategori '%s' diabaikan.", getName(), agencyName, category));
            return;
        }

        if(!s.acceptsCategory(category)){
            System.out.println(String.format("Pengguna %s tidak menemukan kategori '%s' pada agensi %s.", getName(), category, agencyName));
            return;
        }

        if(s.removeCategory(category)){
            System.out.println(String.format("Pengguna %s menghapus kategori '%s' dari agensi %s.", getName(), category, agencyName));
            return;
        }else{
            System.out.println(String.format("Pengguna %s tidak dapat menghapus kategori '%s' dari agensi %s.", getName(), category, agencyName));
            return;
        }
        // KETERANGAN:
    ...
    public void setDeliveryMethod(String agencyName, DeliveryMethod method) {
        ...
        if(s.getDeliveryMethod() == method ){
            System.out.println(String.format("Pengguna %s tetap menggunakan metode %s untuk agensi %s.", getName(), method, agencyName));
            return;
        }

        System.out.println(String.format("Pengguna %s mengubah metode agensi %s dari %s menjadi %s.", getName(), agencyName, s.getDeliveryMethod(), method));
        s.setDeliveryMethod(method);
    ...
    public void receive(String agencyName, String message, String category) {
        ...
        if(s != null && s.acceptsCategory(category)){
            System.out.println(String.format("[%s] dari %s melalui %s - %s: %s", getName(), s.getAgencyName(), s.getDeliveryMethod(), category, message));
        }
        // KETERANGAN:
