...
public class Podcast implements Playable, Downloadable {
    ...
    public Podcast(String title, int duration, String host, int episodeNumber){
        ...
        isPlaying = false;
        downloadProgress = 0;
        fileSize = (duration/60) * 1024 * 1024;
        if(getEpisodeNumber() > 100) {
            fileSize*=1.5;
    }
    }
    ...
    @Override
    public void play() {
        System.out.println(String.format("Memutar podcast: %s - Episode %d by %s", getTitle(), getEpisodeNumber(), getHost()));
        isPlaying = true;
    }
    ...
    public String getDownloadPath() {
        return ("/downloads/podcasts/"+getTitle().replace(" ", "_") + "_ep"+ getEpisodeNumber() + ".mp3");
    }
