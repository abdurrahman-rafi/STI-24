public abstract class Chatbot{
    ...
    public final String reply(UserMessage message) {
        ...
        UserMessage msg;
        if(message == null){
            msg = new UserMessage("", null);
        }else if(message.getType() == null){
            msg = new UserMessage(message.getContent(), null);
        }else{
            msg = message;
        }

        String response = craftResponse(msg.getType(), msg);
        if(response == null){
            return respondToUnknown(msg);
        }
        response = responseStyle.format(response);

        return this.deliveryChannel.deliver(response, this);
    }
    ...
    protected String craftResponse(MessageType type, UserMessage message) {
        ...
        MessageType mtype = (type == null) ? MessageType.UNKNOWN : type;
