...
public abstract class Pokenimons {
  ...
  public void takeDamage(int dmg) {
    int dmgTaken = (dmg > 0) ? dmg : 0;
    setHp(getHp() - dmgTaken);
    if(getHp() < 0){
      setHp(0);
    }

    System.out.println(name + " took " + dmgTaken + " dmg (HP: " + getHp() +")");
  }
  ...
  public void levelUp() {
    setLevel(getLevel() + 1);
    setHp(getHp() + 5);
    System.out.println(name + " leveled up to "+ getLevel() + "!");
  }
  ...
  public final void attack(Pokenimons target) {
    if(isFainted()){
      ...
      String attack = (this instanceof RangedAttacker) ? "RANGED" : "MELEE";
      int damage = (int) ( 10 * typeMultiplierAgainst(target.getType()) );
      System.out.println("[" + attack + "] " + getName() + " uses Tackle on " + target.getName() + " (x" + fmt(typeMultiplierAgainst(target.getType())) + ")" );
  ...
  public final void useSkill(Pokenimons target) {
    if(isFainted()){
      ...
      int damage = (int)(skillBaseDamage() * typeMultiplierAgainst(target.getType()));

      System.out.println(getName() + " uses " + skillName() + " on " + target.getName() + " (x" + fmt(typeMultiplierAgainst(target.getType())) + ")");
      target.takeDamage(damage);
    }
  }
  ...
  public final void printDescription() {
    String classT = this.getClass().getSimpleName();
    System.out.println(String.format("%s @%s (%s) L%d HP=%d", classT, getName(), getType().toString(), getLevel(), getHp()));
  }
}
