...
public class HitlerPlayer extends Player {
  ...
  public Vote vote(Player chancellor, Player president) {
    ...
    if (rand.nextBoolean()) {
        ...
        return Vote.NAY;
    }// STUDENT TODO: Replace with YAY or NAY
  }
  ...
  public Player chooseChancellor(Player[] availablePlayers) {
    ...
    if (candidates.isEmpty()) {
        return null; // No one to choose
    }

    return candidates.get(rand.nextInt(candidates.size())); // STUDENT TODO: Replace with chosen player
  }
  ...
  public Player choosePlayerToKill(Player[] availablePlayers) {
    ...
    if (targets.isEmpty()) {
        return null; // No one to kill
    }

    return targets.get(rand.nextInt(targets.size())); // STUDENT TODO: Replace with chosen player
  }
}