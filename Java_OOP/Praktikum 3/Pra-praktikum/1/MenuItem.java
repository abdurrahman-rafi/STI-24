 ...
 */
 ...
 public abstract class MenuItem {
    ...
    public MenuItem(String namaMenu, int quantity, Unit unit, Kategori kategori) {
        this.namaMenu = (namaMenu != null) ? namaMenu.trim() : "Unknown";
     ...
     */
    public int price() {
        // double base = Math.max(0.0, (double) this.basePrice());
        // double pajak = Math.round(base*TAX_RATE);

        // double hargaPromo = Math.round(base * (100 - promoPercent) / 100.0);
        // if(happyHour == true){
        //     hargaPromo = Math.round(hargaPromo * 0.80);
        // }
        
        
        // return (int) Math.round(hargaPromo + pajak);
        int hargaDasar = Math.max(0, basePrice());
        int harga = (int) ((hargaDasar * (100 - promoPercent)) / 100);
        if (happyHour) harga = (int) (harga * 0.8);
        harga = harga + (int) (hargaDasar * TAX_RATE);
        return harga;
    }
     ...
     */
    public String label() {
        return String.format("%s %d%s", namaMenu, quantity, this.unitSymbol());
    }
    ...
    public String label(boolean showPrice) {
        if(showPrice){
            return String.format("%s %d%s | Rp%d", namaMenu, quantity, this.unitSymbol(), this.price());
        }else{
            return label();
        }
    }
    ...
    return String.format("%s %s | Rp%d dari %s", getKategori(), label(), price(), brand());
    }
    // public String toString() {
    //     return String.format("%s %s %d%s | Rp%d dari %s", kategori, namaMenu, quantity,this.unitSymbol(), this.price(), brand());
    // }
}